...
export function TeamDetails({ team, open, onClose }: TeamDetailsProps) {
  ...
  const exportToCSV = () => {
    ...
    const rows = members.map((m) => [
      ...
      m.joined_at ? format(new Date(m.joined_at), "d MMM yy") : "",
    ]);
    ...
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent className="w-full sm:max-w-xl">
        <SheetHeader>
          ...
          <SheetDescription>
            {team?.member_count} members • Last synced{" "}
            {team?.last_synced_at ? format(new Date(team.last_synced_at), "PPp") : "never"}
          </SheetDescription>
        </SheetHeader>
